...
* Needs pointer to Json Document instance to write to.  Uses WiFi
* library to check connection.
*************/
void HTTP::makeRequest(String url, JsonDocument *doc) { 
  if(WiFi.status() == WL_CONNECTED) { // Make sure wifi is still connected

    // Deserialise response string into a JsonDocument (kind of like a map but weirder for some reason)
